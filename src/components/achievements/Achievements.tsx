      ...
      <div className={theme.container}>
        ...
        <div className="flex flex-col">
          {ACHIEVEMENTS_DATA.map((item, i) => (
            <Reveal key={item.title} delay={ANIMATION_TIMING.revealDelay + (i + 1) * ANIMATION_TIMING.revealStagger}>
              <div
                className={`border-t border-border py-8 md:py-11 group transition-colors duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] hover:border-primary/40 ${
              ...
              >
                <div className="flex flex-col sm:flex-row justify-between items-start gap-3 sm:gap-6">
                  <div className="w-full sm:w-[72%]">
                    <h3 className="font-display text-2xl md:text-[2rem] uppercase leading-none group-hover:text-primary transition-colors duration-500 ease-[cubic-bezier(0.22,1,0.36,1)]">
                      {item.title}
                    </h3>
                    <p className={`${theme.bodyText} mt-3 md:mt-4 max-w-xl`}>{item.desc}</p>
                  </div>
                  <span className="text-muted text-sm uppercase tracking-[0.16em] group-hover:text-primary transition-colors duration-500 ease-[cubic-bezier(0.22,1,0.36,1)]">
                    {item.date}
                  </span>
                </div>
              </div>
